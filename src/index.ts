...
class Game {
  ...
  public startGame(min: number, max: number): void {
    this.randomNumber = new NumberGenerator(min, max).generateNumber();
    this.attempts = 0;
    this.gameOver = false;

    console.log(
      `Welcome to the Guess the Number game! A random number between ${min} and ${max} has been generated.`
    );
  }

  public async playGuess() {
    ...
    if (userGuess === this.randomNumber) {
      console.log(
        `Congratulations! You guessed the number correctly in ${this.attempts} attempts.`
      );
      this.gameOver = true;
